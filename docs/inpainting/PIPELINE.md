...
### [7-b] 가장자리만 보이는 물체 — 참조 프레임에서 붙여넣기

`paste_object_from_reference.py` — `--paste "object_01"`로 켠다.

`complete_occluded_objects`는 보이는 픽셀의 **볼록 껍질** 안에서만 채운다. 손잡이와 아래
테두리만 보이는 머그컵에서는 그 껍질이 몸통을 감싸지 못하고 사이의 얇은 영역만 덮는다.
프롬프트를 고쳐도, `--max_hull_ratio`를 3.2 → 6.0으로 올려도 복원량이 6,808 → 7,030 px로
거의 그대로였다. 알고리즘의 한계지 설정 문제가 아니다.

정적 카메라라 물체는 손이 닿기 전 온전히 보인다. 이 도구는 그 **깨끗한 프레임의 실루엣을
현재 프레임으로 워핑**한다(마스크 모멘트 유사변환 → ECC 정밀화). 실제로 보이는 픽셀은
그대로 두고 가려진 부분만 대체하며, force-front 마스크도 함께 쓰면서 엄지를 도려낸다.

IMG_5395의 머그: 깨끗한 참조 27프레임에서 **261,535 px** 복원. 강제 물체 전경이
255,667 → 645,206 px로 늘고 컵 몸통이 온전히 그려진다.

어느 물체에 필요한지는 아직 자동 판정이 없다. 복원 후 마스크가 그 물체의 깨끗한 프레임
면적보다 훨씬 작으면 이 단계가 필요하다는 신호이고, 그 규칙을 넣으면 `--paste`도 자동이 된다.
