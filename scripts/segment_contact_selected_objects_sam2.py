...
def discover_episodes(
    ...
    *,
    ...
    # ``aggregate_finger_contact.py`` stores its sequence-level summary next
    # to the per-frame HaCo files.  It is metadata, not an extra video frame.
    files = sorted(
        path for path in contact_dir.glob("*.npz")
        if path.name != "finger_contact.npz"
    )
