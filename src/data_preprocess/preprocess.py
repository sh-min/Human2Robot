...
def _load_model_frames(
    ...
    if source.is_dir():
        ...
        for frame_index in sorted(needed):
            ...
            decoded[frame_index] = preprocess_rgb_frame(
                rgb, crop_size, spatial_profile
            )
    else:
        capture = cv2.VideoCapture(str(source))
        frame_index = 0
...
def main() -> None:
    ...
    for rec_dir in recordings:
        ...
        if not rgb_dir.is_dir():
            raise FileNotFoundError(f"{rec_dir}: missing rgb/")
        source_fps = _resolve_source_fps(
            rec_dir, args.source_fps, args.sampling_profile
        )
        original_count = source_frame_count(rgb_dir)
        alignment = build_token_alignment(
            original_count,
            source_fps,
        ...
        if output.exists() and not args.overwrite:
            ...
            continue
        jobs.append((rec_dir, alignment, provenance))
    ...
    feature_extractor = VJEPAFeatureExtractor(encoder, pool="none").to(device)

    for job_index, (rec_dir, alignment, provenance) in enumerate(jobs, 1):
        print(
        ...
        vjepa_orig, decoded_count = extract_vjepa(
            rec_dir / "rgb",
            feature_extractor,
            device,
            args.crop_size,
        ...
        if mano_payload is None:
            raise FileNotFoundError(
                f"{rec_dir}: missing result.json and rgb_hawor/retarget_input.npz"
            )
