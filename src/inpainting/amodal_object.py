"""Stage 8b: video amodal segmentation of the object with Diffusion-VAS.

The SAM2 mask from segment_object.py (stage 8a) is *modal* — only the
visible object pixels, so gripping fingers bite chunks out of it. Diffusion-
...
This stage predicts and temporally regularizes the object's amodal silhouette.

This script must run in the `diffusion_vas` conda env (NOT `inpaint`):
    conda run -n diffusion_vas python amodal_object.py --processed_demo <pd>

Inputs:
    <pd>/video_L.mp4
    <pd>/object_layer/object_mask_raw.npy        (T,H,W) bool — SAM2 modal mask

Outputs:
    <pd>/object_layer/object_rawsum.npy          (T,H,W) float16 — raw channel sums
    <pd>/object_layer/object_mask_amodal.npy     (T,H,W) bool — amodal silhouette
    <pd>/object_layer/object_amodal_overlay.mp4  raw + amodal outline (debug)

Per-window checkpoints are saved under <pd>/object_layer/_amodal_ckpt/ so the
script can resume after OOM crashes without re-running completed windows.

Usage:
    python amodal_object.py --processed_demo /result/skill2policy/processed/cam0/0 \\
...
def main() -> None:
    ...
    ap.add_argument("--assemble_only", action="store_true",
                    help="skip inference; load all window checkpoints, aggregate "
                         "and post-process into object_mask_amodal.npy")
    args = ap.parse_args()
    ...
    modal = np.load(pd / "object_layer" / "object_mask_raw.npy").astype(bool)
    ...
    ckpt_dir = pd / "object_layer" / "_amodal_ckpt"
    ...
    np.save(out_dir / "object_rawsum.npy", avg_rawsum.astype(np.float16))
    print(f"[ok] wrote {out_dir / 'object_rawsum.npy'}")
    ...
    np.save(out_dir / "object_mask_amodal.npy", amodal_mask)

    # Debug overlay video
    overlay = rgb.copy()
    for t in range(T):
        overlay[t][_outline(amodal_mask[t])] = (255, 0, 0)
    _write_video(out_dir / "object_amodal_overlay.mp4", overlay, args.fps)

    per = amodal_mask.sum(axis=(1, 2))
    modal_per = modal.sum(axis=(1, 2))
    print(f"[ok] wrote {out_dir / 'object_mask_amodal.npy'}")
    print(f"[ok] wrote {out_dir / 'object_amodal_overlay.mp4'}")
