...
Each layer keeps its actual content (robot rgb / cube from bg) but gets a
color tint so you can tell them apart:
    front-MCP robot  →  blue tint
    cube (isolated)  →  green tint
...
TINT_CUBE   = np.array([ 60, 255, 120], dtype=np.float32)  # green
...
def main() -> None:
    ...
    print(f"[info] T={T}, {W}x{H}, depth_bias={args.depth_bias}, "
          f"tints: front=blue, cube=green, behind=red")
    ...
    for t in range(T):
        ...
        # layer 2: cube (bg at cube_mask pixels), green tint
        if cube_m[t].any():
            frame[cube_m[t]] = bg[t][cube_m[t]]
            _tint(frame, cube_m[t], TINT_CUBE)
    ...
    print("    layer key:  blue=front-MCP robot   green=cube   red=behind-MCP robot")
