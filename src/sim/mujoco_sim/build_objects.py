...
capsules.
  ...
  milk_carton      55 x 55 x 115 mm gable-top carton (75 mm body, 25 mm
                   triangular roof and 15 mm sealed fin)
  pringles         O65 x 90 mm Pringles can
  cup_green        O80 x 105 mm cup with a 60 x 25 mm handle
  cup_blue         O75 x 85 mm cup with a 65 x 30 mm handle
...
MILK_BODY_H = 0.075   # rectangular part
MILK_GABLE_H = 0.025  # triangular/sloped roof
MILK_ROOF_T = 0.009   # roof slab thickness (thick enough to fill the gable)
MILK_FIN_H = 0.015    # thin sealed fin on the ridge -> 115 mm total
...
# --- cups ------------------------------------------------------------
# ``handle_h`` is the full outside height and ``handle_out`` is how far the
# handle projects from the outside of the cup. ``rod`` is the capsule radius.
# The handle ends are sunk 3 mm into the wall so they are welded to the cup.
CUP_WALL = 0.004
CUP_BASE = 0.006
CUP_SEGS = 24
CUP_HANDLE_SEGS = 8
...
CUPS = {
    "cup_green": dict(r=0.0400, h=0.105, handle_h=0.060,
                      handle_out=0.025, rod=0.0048,
                      rgb=(0.20, 0.62, 0.34)),
    "cup_blue": dict(r=0.0375, h=0.085, handle_h=0.065,
                     handle_out=0.030, rod=0.0048,
                     rgb=(0.15, 0.40, 0.75)),
}
...
def build_milk_carton() -> str:
  ...
  <!-- AUTO-GENERATED by build_objects.py. Do not edit by hand.
       {MILK_W*1000:.0f} x {MILK_W*1000:.0f} x {total*1000:.0f} mm gable-top carton.
       Measured height split: {MILK_BODY_H*1000:.0f} mm body + {MILK_GABLE_H*1000:.0f} mm triangular roof
       + {MILK_FIN_H*1000:.0f} mm sealed fin. Total mass is 0.282 kg.
...
def build_cup(name, *, r, h, handle_h, handle_out, rod, rgb) -> str:
    r_mid = r - CUP_WALL / 2
    ...
    # Elliptical centerline lets the measured outside height and outward
    # projection be represented independently. The 3 mm term compensates for
    # the attachment centerline being sunk into the cup wall.
    arc_x = handle_out - rod + 0.003
    arc_z = handle_h / 2 - rod
    if arc_x <= 0 or arc_z <= 0:
        raise ValueError(f"invalid handle dimensions for {name}")
    ...
    for i in range(CUP_HANDLE_SEGS + 1):
        ...
        pts.append((cx + CUP_HANDLE_DIR * arc_x * math.cos(t), 0.0,
                    zc + arc_z * math.sin(t)))
    handle = [
  ...
  <!-- AUTO-GENERATED by build_objects.py. Do not edit by hand.
       ...
       plus a {handle_h*1000:.0f} mm high handle projecting {handle_out*1000:.0f} mm
       from the cup wall. Origin at bottom center. -->
