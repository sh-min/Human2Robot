...
class RobotIdentityLockTests(unittest.TestCase):
    def test_all_rb5_links_are_mandatory(self):
        self.assertEqual(renderer.RB5_LINK_NAMES, tuple(f"link{index}" for index in range(7)))

    def test_material_is_fixed(self):
        self.assertEqual(renderer.RB5_ARM_COLOR, (0.72, 0.72, 0.72))
        self.assertEqual(renderer.RB5_LIGHT_COLOR, (1.0, 1.0, 1.0))

    def test_input_sidecar_must_declare_xhand(self):
        data = {
            "rb5_q": np.zeros((1, 6)),
            "safety_constraints_enforced": np.bool_(True),
            "wrist_pos": np.zeros((1, 3)),
            "wrist_rot": np.zeros((1, 3, 3)),
            "qpos": np.zeros((1, 12)),
        ...
        metadata = {
            ...
            "embodiment": "xhand",
            "trajectory_constraints": {"enforced": True},
        }

        self.assertEqual(renderer.validate_input(data, metadata)[:2], (1, "right"))
        metadata["embodiment"] = "another_hand"
        with self.assertRaisesRegex(ValueError, "embodiment='xhand'"):
            renderer.validate_input(data, metadata)
