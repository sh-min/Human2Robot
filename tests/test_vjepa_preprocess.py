...
class VjepaAlignmentTests(unittest.TestCase):
    def test_color_jitter_is_recording_deterministic_and_pixel_level(self):
        first = preprocess.sample_color_jitter(
            "episode", 42, brightness=0.2, contrast=0.2,
            saturation=0.2, hue=0.05,
        )
        second = preprocess.sample_color_jitter(
            "episode", 42, brightness=0.2, contrast=0.2,
            saturation=0.2, hue=0.05,
        )
        ...
        image[..., 0] = 220
        image[..., 1] = 80
        jittered = preprocess.apply_color_jitter(image, first)
        self.assertEqual(jittered.shape, image.shape)
        self.assertEqual(jittered.dtype, np.uint8)
        self.assertFalse(np.array_equal(jittered, image))

    def test_zero_color_jitter_is_an_identity(self):
        self.assertIsNone(preprocess.sample_color_jitter("episode", 42))
        image = np.arange(60, dtype=np.uint8).reshape(4, 5, 3)
        np.testing.assert_array_equal(
            preprocess.apply_color_jitter(image, None), image
        )

    def test_annotated_video_prefix_drops_only_extra_tail_frames(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "gt_labels.json").write_text(
                json.dumps({"num_frames": 24, "fps": 24.0})
            )
            video = root / "rgb"
            video.touch()
            original = preprocess.source_frame_count
            try:
                preprocess.source_frame_count = lambda _: 25
                self.assertEqual(
                    preprocess.aligned_source_frame_count(root, video), 24
                )
                preprocess.source_frame_count = lambda _: 23
                with self.assertRaisesRegex(ValueError, "GT needs 24"):
                    preprocess.aligned_source_frame_count(root, video)
            finally:
                preprocess.source_frame_count = original
